...
export type EscrowState = { discriminator: ReadonlyUint8Array; version: number; bump: number; vaultBump: number; roomId: ReadonlyUint8Array; host: Address; mint: Address; vault: Address; fundingTarget: bigint; totalContributed: bigint; participantCount: number; contributorCount: number; participants: Array<Address>;  };

export type EscrowStateArgs = { version: number; bump: number; vaultBump: number; roomId: ReadonlyUint8Array; host: Address; mint: Address; vault: Address; fundingTarget: number | bigint; totalContributed: number | bigint; participantCount: number; contributorCount: number; participants: Array<Address>;  };

export function getEscrowStateEncoder(): FixedSizeEncoder<EscrowStateArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['version', getU8Encoder()], ['bump', getU8Encoder()], ['vaultBump', getU8Encoder()], ['roomId', fixEncoderSize(getBytesEncoder(), 8)], ['host', getAddressEncoder()], ['mint', getAddressEncoder()], ['vault', getAddressEncoder()], ['fundingTarget', getU64Encoder()], ['totalContributed', getU64Encoder()], ['participantCount', getU8Encoder()], ['contributorCount', getU8Encoder()], ['participants', getArrayEncoder(getAddressEncoder(), { size: 4 })]]), (value) => ({ ...value, discriminator: ESCROW_STATE_DISCRIMINATOR }));
}

export function getEscrowStateDecoder(): FixedSizeDecoder<EscrowState> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['version', getU8Decoder()], ['bump', getU8Decoder()], ['vaultBump', getU8Decoder()], ['roomId', fixDecoderSize(getBytesDecoder(), 8)], ['host', getAddressDecoder()], ['mint', getAddressDecoder()], ['vault', getAddressDecoder()], ['fundingTarget', getU64Decoder()], ['totalContributed', getU64Decoder()], ['participantCount', getU8Decoder()], ['contributorCount', getU8Decoder()], ['participants', getArrayDecoder(getAddressDecoder(), { size: 4 })]]);
}

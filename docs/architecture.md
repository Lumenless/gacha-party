...
- Existing room v3 and escrow v5 accounts do not contain the automatic-membership authority and must be replaced with newly created demo parties after this devnet upgrade. PDA derivations are unchanged.
- Room v4 and escrow v6 are deployed on devnet under explicit hackathon release authorization and passed the combined MagicBlock/escrow smoke. Independent review remains required before mainnet or production-value funds.
